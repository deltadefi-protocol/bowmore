...
use crate::{config::AppConfig, scripts::{lp_token::lp_token_mint_blueprint, plutus_loader::get_compiled_code_by_index}};

pub fn withdrawal_intent_mint_blueprint(oracle_nft: &str) -> Result<MintingBlueprint, whisky::WError> {
    ...
    .param_script(
        &compiled_code,
        &[&byte_string(oracle_nft).to_string(), &byte_string(&lp_token_mint_blueprint.hash).to_string()],
...
pub fn withdrawal_intent_spend_blueprint(oracle_nft: &str) -> Result<SpendingBlueprint, whisky::WError> {
