...
pub fn get_compiled_code_by_index(index: usize) -> Result<String, WError> {
    ...
    if index >= validators_array.len() {
        return Err(WError::new(&format!("Index {} out of bounds for validators array", index), "IndexError"));
    }
    
    let validator = &validators_array[index];
    let compiled_code = validator["compiledCode"]
        .as_str()
        .ok_or_else(|| WError::new(&format!("No compiledCode found at index {}", index), "JsonError"))?;
